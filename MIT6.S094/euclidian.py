...
# Calculate Euclidian distance using numpy arrays
# euclidian_distance = np.sqrt(np.sum((np.array(features) - np.array(predict))**2))

dataset = {'k': [[1, 2], [2, 3], [3, 1]], 'r': [[6, 5], [7, 7], [8, 6]]}
new_features = [5, 7]

# Scatter plots onto graph
# [[plt.scatter(ii[0], ii[1], s=100, color=i) for ii in dataset[i]] for i in dataset]

# Add new feature to graph
...
def k_nearest_neigbors(data, predict, k=3):
    ...
    print('Counter Vote Most Common: ', Counter(votes).most_common(1))
